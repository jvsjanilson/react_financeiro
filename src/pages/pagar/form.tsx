...
const PagarForm: React.FC = () => {
  ...
  const handleSubmit = (e: any) => {
    ...
    if (id) {
      pagarService.update(id, data).then( data => {
            navigate('/pagars')
        }).catch(err => {
      ...
      pagarService.create(data).then( data => {
            navigate('/pagars')
        }).catch(err => {
            if (err.response.status === 400) {
                setErrors(err.response.data);
  ...
  return (
    <div>
      ...
      <Form onSubmit={handleSubmit} noValidate>
          <Card>
              <Card.Header>
                <Card.Title className="text-center">Formulário de Conta a pagar</Card.Title>
              </Card.Header>
              ...
              <Card.Body>
                ...
                <Link to="/pagars" className="btn btn-secondary ms-1"> <FontAwesomeIcon icon={faArrowLeft} /> Voltar</Link>
                </Card.Footer>
          </Card>
      </Form>
    </div>
